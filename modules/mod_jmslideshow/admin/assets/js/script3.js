...
function JMSlideShow_SourceChange(source){
  jQuery(".s1, .s2, .s3, .s4, .s5, .s6, .s7, .s8, .s9, .s10").not(".s"+source).parents('.control-group').css({
    display:'none'
  });
...
function JMSlideShow_PagertypeChange(source){
	switch(source){
    	case '1':
    		jQuery('#jform_params_jmslideshow_image_thumbnail_width').parents('.control-group').css({
    	        display:'none'
    	    });
    		jQuery('#jform_params_jmslideshow_image_thumbnail_height').parents('.control-group').css({
    	        display:'none'
    	    });
    		break;
    	case '2':
