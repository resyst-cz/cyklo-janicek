...
jQuery.fn.getHexColor = function(rgb) {
    ...
    if(rgb == null){
       //var rgb_img = jQuery("[data-index="+this[0].dataset['index']+"]").find("div#img_t").css("background-color");
       var rgb_img = this.find("div#img_t").css("background-color");
       rgb = rgb_img.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/);
    }
...
var objJ = {
    ...
    Input: function (options, name, currentValue, defaultValue, hasPx){
        ...
        this.getValue = function($i) {
            if (this.type == 'checkbox') {
                return $i.is(':checked') ? this.value : newDefaultValue[this.name];
            } else {
                return $i.val();
            }
        }
...
jElement.prototype.getSetProperty = function(css) {
    ...
    switch (css) {
        ...
        case 'height':
            ...
            setProperty = this.$el.children("div#img_t")[css]();
            break;
        ...
        case 'padding':
           // setProperty = this.$el.find('img:first').css('padding-top');
            setProperty = jQuery('div#img_t img').css('padding');
            break;
        default:
            setProperty = this.$el.css(css);
              if(css == "textDecoration"){
                    var valSp = setProperty.split(' ');
                    setProperty = valSp[0];
              }
    ...
    if(typeof tab == 'string'){
        ...
        for (var i in css) {
            if(typeof css[i] == 'string'){
                ...
                if(tab == "label"){
                    var getStyleLabels = jQuery("[data-index="+this.$el[0].dataset['index']+"]").find("label").css(input['name']);
                    //console.log(this.$el.getHexColor(setProperty));
                    if(getStyleLabels){
                        if(typeof getStyleLabels != "number"){ 
                            getStyleLabels = (getStyleLabels.indexOf('px') == 2)?parseInt(getStyleLabels):getStyleLabels;
                        }

                        if(input['name'] == "color"){
                            getStyleLabels = this.$el.getHexColor(getStyleLabels);
                        }
                        console.log(getStyleLabels);
                        input['currentValue'] = (css[i] == "underline")?getStyleLabels.split(' ')[0]:getStyleLabels;
                        //getStyleLabels = getStyleLabels.indexOf('px')?getStyleLabels:parseInt(getStyleLabels);
...
jElement.prototype.renderTooltipContent = function() {
    ...
    /////tabs
    jQuery("#tabs").find("div[id^='tab']").css('display','none');
    jQuery('.on_tab:first').addClass('ui-tabs-active ui-state-active');
    jQuery(jQuery('.on_tab:first').children('a').attr('id')).css('display','block');
    jQuery('.on_tab').click(function(){
        jQuery(".on_tab").each(function(){
            this.removeClass('ui-tabs-active ui-state-active');
            var id_c = jQuery(this).children('a').attr('id');
            jQuery(id_c).css('display','none');

        });
        this.addClass('ui-tabs-active ui-state-active');
        jQuery(jQuery(this).children('a').attr('id')).css('display','block');
    });
...
jElement.prototype.getChangedCss = function() {
    ...
    _.each(this.tabs, function(tab, index) {
        _.each(globalTabs[tab].css, function(cssIndex) {
            ...
            if(tab == "image"){
                value = this.$el.children("div#img_t").css(jQuery.camelCase(cssName));
                if(cssName == "padding"){
                    value = jQuery("div#img_t img").css(jQuery.camelCase(cssName));
                }

            }
            if(cssName == "text-decoration"){
                var filter_v = value.split(' ');
                value = filter_v[0];
            }
...
jQuery(document).ready(function() {
    ...
    jQuery('#save').click(function(e) {
        ...
        _.each(globalDragElements, function(item) {
            ...
            dataToServer.label[index] = jQuery("[data-index="+item.$el[0].dataset["index"]+"]").find("label").attr('style');

            //console.log(jQuery("[data-index="+item.$el[0].dataset["index"]+"]").find("label").attr('style'));
...
function init(dragElements, dropElements) {
    ...
    if (dataFromServer) {
        ...
        for (var i in dataFromServer.css) {
            if(jQuery("[data-index="+i+"] div#img_t").length){
                var marginData = '';
                var imgData = '';
                var imgMargin = '';
                var imgHeight = '';
                for(var a in dataFromServer.css[i]){
                    if(a.indexOf("margin") > -1){
                        marginData += a+": "+dataFromServer.css[i][a]+"; ";
                    }else if(a != 'padding'){
                        imgData += a+": "+dataFromServer.css[i][a]+"; ";
                        if(a == 'height'){
                            imgHeight = dataFromServer.css[i][a];
                    ...
                    if(a == 'padding'){
                        //imgMargin =  parseInt(dataFromServer.css[i][a]) + Math.floor((parseInt(imgHeight)-2*parseInt(dataFromServer.css[i][a]) - jQuery("[data-index="+i+"]").find("div#img_t img").height())/2);
                        imgMargin =  parseInt(dataFromServer.css[i][a]);
                    }
                }

                jQuery("[data-index="+i+"]").find("div#img_t").attr("style", imgData);
                jQuery("[data-index="+i+"]").attr("style", marginData);

                jQuery("[data-index="+i+"]").find("div#img_t img").css("padding", imgMargin+"px");

            }else{
                applyCss(i, dataFromServer.css[i]);
                jQuery("[data-index="+i+"]").find("label").attr("style", dataFromServer.label[i]);
