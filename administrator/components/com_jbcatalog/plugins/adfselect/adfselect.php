...
class AdfSelectPlugin {
    private function _getID(){
         $db = JFactory::getDbo();
         $db->setQuery("SELECT id FROM #__jbcatalog_plugins WHERE name = 'adfselect'");
         return $db->loadResult();
    }
    //when this type selected
    public function getAdfViewJSSelected(){
        $js['select'] = 'if(obj.value == '.$this->_getID().'){
    ...
    public function getItemEdit($params = array()){
        ...
        return JHTML::_('select.genericlist',   $selarr, 'extraf['.$adfid.']', 'class="inputboxsel" size="1"', 'id', 'name', $value );
